...
def load_heart():
    ...
    y = df["target"]
    X["age_group"] = pd.cut(X["age"], bins=[0,40,55,70,100],
...
def train_heart_nn():
    ...
    history = model.fit(Xtr, y_train, validation_data=(Xval, y_val),
...
def train_titanic_nn():
    ...
    history = model.fit(Xtr, y_train, epochs=100, batch_size=16,
